...
public class ScoreBoardManager {
	...
	public static void init(Player player) {
		...
		SimpleScoreboard ss = new SimpleScoreboard(scheme.getSecondary()+"»"+scheme.getPrimary()+Main.INSTANCE.getConfig().getString("serverapi.serverconfig.servername")+scheme.getSecondary()+"«");
		int max = generateScoreboard(player,scheme).size();
		for(int i =0;i < max;i++)
		    ss.add(generateScoreboard(player,scheme).get(i), max-i);
		ss.update();
		ss.send(player);
	...
	public static void update(Player player) {
        final ColorScheme scheme = Main.currentGame.getColorScheme();
	...
	public static ArrayList<String> generateScoreboard(Player player,ColorScheme scheme){
